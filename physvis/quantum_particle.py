...
class QuantumParticle:
    ...
    def x(self, x):
        ...
        self.k = space.normal(Ns, self._dks)
        k0 = self.k[0, 0]

        self._k0_dot_x = (k0*self.x).sum(axis=-1)
        self._mod_factor = np.exp(-1j*self._k0_dot_x)*self._dxs[0]/(2*np.pi)
        self._unmod_factor = np.exp(1j*self._k0_dot_x)*(2*np.pi)/self._dxs[0]
        self._k2 = (self.k**2).sum(axis=-1)

    @property
    def psi_x(self):
        return self._psi_mod_x*self._unmod_factor

    @psi_x.setter
    def psi_x(self, psi_x):
        self._psi_mod_x = self._mod_factor*psi_x
